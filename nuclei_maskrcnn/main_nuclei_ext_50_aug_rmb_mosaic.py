...
epoch_number_init_head = 15
epoch_number_init_4plus = 15
epoch_number_init_all = 15
epoch_number_iter = 0
...
train_ids_mosaic = glob.glob(TRAIN_DATA_MOSAIC_PATH+'/*/images/*.png') # mosaic

train_ids.extend(train_ids_ext)
train_ids.extend(train_ids_mosaic) # mosaic

test_ids = next(os.walk(TEST_DATA_PATH))[1]
test_ids_m = next(os.walk(TEST_DATA_MOSAIC_PATH))[1]

print(len(train_ids),len(train_ids_ext),len(train_ids_mosaic))
...
if train_flag:
    ...
    if train_all:
        ...
        epoch_init = epoch_init + epoch_number_init_all
