...
class TrainingConfig(Config):
    ...
    # Number of classes (nuclei, nuclei surrounding and bg)
    NUM_CLASSES = 1 + 2
...
class NucleiDataset(utils.Dataset):
    ...
    def load_mask(self, image_id):
        ...
        class_ids_dil = 2*np.ones(len(mask_files), np.int32)
        class_ids = np.concatenate((class_ids, class_ids_dil))
        mask = np.zeros([mask0.shape[0], mask0.shape[1], num_inst])
        for k in range(num_inst):
            mask[:, :, k] = skimage.io.imread(os.path.join(mask_dir, mask_files[k]))

        train_id = os.path.dirname(self.image_info[image_id]['path'])
        train_id = train_id.split('/')[-2]
        masks_dil = np.load(TRAIN_DIL_DATA_PATH+'/'+train_id+'_dil_masks.npy')
        mask = np.concatenate((mask,masks_dil),axis=2)
        return masks_dil, class_ids_dil
...
dataset_train.add_class("cell", 2, "nulcei_surrounding")
...
dataset_val.add_class("cell", 2, "nulcei_surrounding")
...
dataset_test.add_class("cell", 2, "nulcei_surrounding")
