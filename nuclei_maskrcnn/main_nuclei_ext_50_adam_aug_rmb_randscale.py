...
epoch_number_head = 30
epoch_number_all_fast = 8
...
class InferenceMosaicConfig(TrainingConfig):
    IMAGE_MIN_DIM = 512
    IMAGE_MAX_DIM = 1024
...
inference_config_mosaic = InferenceMosaicConfig()
...
for k, test_id_m in enumerate(test_ids_m):
    dataset_test_m.add_image("cell", k,
                             os.path.join(TEST_DATA_MOSAIC_PATH, test_id_m, 'images', test_id_m + '_image.png'))
...
if train_flag:
    ...
    if train_head:
        ...
        epoch_add = 0
        model.train(dataset_train, dataset_val, learning_rate=config_head.LEARNING_RATE, epochs=epoch_init,
                    layers='heads')
        while epoch_add < epoch_number_iter:
            ...
            if epoch_add >= 2 and val_mAP[-1] < val_mAP[-2] and val_mAP[-1] < val_mAP[-3]:
                break
            epoch_add += 1
            model.train(dataset_train, dataset_val, learning_rate=config_head.LEARNING_RATE,
                        epochs=epoch_init + epoch_add, layers='heads')
        del model
    ...
    if train_all:
        ...
        epoch_add = 0
        model.train(dataset_train, dataset_val, learning_rate=config_all.LEARNING_RATE, epochs=epoch_init_fast,
                    layers="all")
        model.train(dataset_train, dataset_val, learning_rate=config_all.LEARNING_RATE / 10., epochs=epoch_init_slow,
                    layers="all")
        while epoch_add < epoch_number_iter:
            val_mAP.append(compute_mAP_val())
            if epoch_add >= 2 and val_mAP[-1] < val_mAP[-2] and val_mAP[-1] < val_mAP[-3]:
                break
            epoch_add += 1
            model.train(dataset_train, dataset_val, learning_rate=config_all.LEARNING_RATE / 10.,
                        epochs=epoch_init_slow + epoch_add, layers='all')
        del model
...
###########################################################################################
model_inf = modellib.MaskRCNN(mode="inference", config=inference_config_mosaic, model_dir=MODEL_DIR)
model_path = model_inf.find_last()[1]
# model_path = '/home/jieyang/code/TOOK18/nuclei_maskrcnn/logs/nuclei_train20180211T2323/mask_rcnn_nuclei_train_0019.h5'
assert model_path != "", "Provide path to trained weights"
print("Loading weights from ", model_path)
...
for image_id in dataset_test_m.image_ids:
    ...
    ul_shape = np.load(
        dataset_test_m.image_info[image_id]['path'].replace('images/', '').replace('_image.png', '_ul_shappe.npy'))[0:2]
